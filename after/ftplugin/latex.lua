...
local function render_pdf()
	...
	vim.fn.jobstart([[bash -c 'latexmk -pdf "]] .. file_path .. [["']], { on_exit = callbacks.document_render_finish })
...
local function render_makefile()
	vim.fn.jobstart([[bash -c "make"]], { on_exit = callbacks.document_render_finish })
